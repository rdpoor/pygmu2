...
class TestExpanderPE:
    ...
    def test_create_with_params(self):
        ...
        source = SinePE(frequency=440.0)
        gate = ExpanderPE(
            source, threshold=-30.0, attack=0.0005, release=0.1, gate_range=-60.0
        )
...
class TestCompositeGraphLifecycle:
    ...
    def test_compressor_renderer_reaches_envelope(self, renderer):
        ...
        assert (
            EnvelopePE in pe_types
        ), "Renderer should discover the internal EnvelopePE"
        assert (
            DynamicsPE in pe_types
        ), "Renderer should discover the internal DynamicsPE"
    ...
    def test_compressor_no_manual_lifecycle(self, renderer):
        ...
        assert not hasattr(
            comp, "_reset_state"
        ), "Manual _reset_state should be removed"
