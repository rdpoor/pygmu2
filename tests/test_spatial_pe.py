...
class TestSpatialAdapter:
    ...
    def test_stereo_to_quad(self):
        ...
        expected = np.array(
            [[1.0, 10.0, 5.5, 5.5], [2.0, 20.0, 11.0, 11.0]],  # L, R, (L+R)/2, (L+R)/2
            dtype=np.float32,
        )
        np.testing.assert_array_almost_equal(snippet.data, expected, decimal=5)
        assert snippet.channels == 4
    ...
    def test_quad_to_stereo(self):
        ...
        quad_data = np.array(
            [[1.0, 10.0, 100.0, 1000.0], [2.0, 20.0, 200.0, 2000.0]], dtype=np.float32
        )
...
class TestSpatialLinear:
    ...
    def test_azimuth_clamping(self):
        ...
        np.testing.assert_array_almost_equal(
            snippet_180.data, expected_right, decimal=5
        )
        ...
        np.testing.assert_array_almost_equal(
            snippet_neg180.data, expected_left, decimal=5
        )
...
class TestSpatialConstantPower:
    ...
    def test_constant_power_property(self):
        ...
        for az in azimuths:
            panned = SpatialPE(
                mono_source, method=SpatialConstantPower(azimuth=float(az))
            )
            self.renderer.set_source(panned)
            snippet = panned.render(0, 1)
        ...
        assert all(
            abs(p - powers[0]) < 1e-5 for p in powers
    ...
    def test_partial_right_pan_azimuth_45(self):
        ...
        expected = np.array(
            [[np.cos(pan_angle_rad), np.sin(pan_angle_rad)]], dtype=np.float32
        )
        np.testing.assert_array_almost_equal(snippet.data, expected, decimal=5)
        assert snippet.channels == 2
    ...
    def test_stereo_input_mixed_to_mono_first(self):
        ...
        expected = np.array(
            [[5.5 * np.cos(np.pi / 4), 5.5 * np.sin(np.pi / 4)]], dtype=np.float32
        )
        np.testing.assert_array_almost_equal(snippet.data, expected, decimal=5)
        assert snippet.channels == 2
...
class TestSpatialPEBasics:
    ...
    def test_inputs_includes_source_and_dynamic_params(self):
        ...
        dynamic_pan = SpatialPE(
            mono_source, method=SpatialLinear(azimuth=dynamic_azimuth)
        )
    ...
    def test_extent_passthrough(self):
        ...
        mono_source = ArrayPE(
            np.array([1.0, 2.0, 3.0], dtype=np.float32).reshape(-1, 1)
        )
