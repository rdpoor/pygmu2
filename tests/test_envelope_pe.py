...
class TestEnvelopePERMS:
    ...
    def test_rms_mode_smoother(self):
        ...
        env_peak = EnvelopePE(
            source, attack=0.001, release=0.001, mode=DetectionMode.PEAK
        )
        env_rms = EnvelopePE(
            source, attack=0.001, release=0.001, mode=DetectionMode.RMS
        )
...
class TestEnvelopePEStereo:
    ...
    def test_stereo_independent_channels(self):
        ...
        with self.renderer:
            ...
            np.testing.assert_array_almost_equal(
                snippet.data[:, 0], snippet.data[:, 1], decimal=4
            )
