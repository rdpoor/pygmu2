...
class TestSuperSawPERender:
    ...
    def test_render_single_voice_equals_blit_saw(self):
        ...
        super_saw = SuperSawPE(
            frequency=440.0, voices=1, detune_cents=0.0, randomize_phase=False
        )
        ...
        correlation = np.corrcoef(
            snippet1.data[:, 0].flatten(), snippet2.data[:, 0].flatten()
        )[0, 1]
...
class TestSuperSawPEDetune:
    ...
    def test_detune_produces_beating(self):
        ...
        assert (
            envelope_variation > 0.01
        ), "Should have some amplitude variation from beating"
