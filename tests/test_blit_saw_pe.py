...
class TestBlitSawPEBasics:
    ...
    def test_create_with_all_params(self):
        ...
        saw = BlitSawPE(frequency=220.0, amplitude=0.5, m=15, leak=0.995, channels=2)
...
class TestBlitSawPERender:
    ...
    def test_render_frequency_cycles(self):
        ...
        assert (
            abs(zero_crossings - frequency) <= 5
...
class TestBlitSawPEStateManagement:
    ...
    def test_state_persists_across_contiguous_renders(self):
        ...
        np.testing.assert_array_almost_equal(combined, full_chunk.data, decimal=5)
...
class TestBlitSawPEDifferentSampleRates:
    ...
    def test_sample_rate_48000(self):
        ...
        zero_crossings = sum(
            1 for i in range(1, len(data)) if data[i - 1] < 0 and data[i] >= 0
        )
