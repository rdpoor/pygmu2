...
import numpy as np
from pygmu2 import ConstantPE, NullRenderer
...
class TestConstantPERender:
    ...
    def test_render_correct_value(self):
        ...
        np.testing.assert_array_almost_equal(
            snippet.data, np.full((100, 1), 0.75, dtype=np.float32)
        )
    ...
    def test_render_negative_start(self):
        ...
        np.testing.assert_array_almost_equal(
            snippet.data, np.full((200, 1), 0.5, dtype=np.float32)
        )
    ...
    def test_render_large_value(self):
        ...
        snippet = const.render(0, 100)
        np.testing.assert_array_almost_equal(
            snippet.data, np.full((100, 1), 440.0, dtype=np.float32)
        )
