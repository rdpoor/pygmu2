...
class BiquadPE(ProcessingElement):
    ...
    def _render(self, start: int, duration: int) -> Snippet:
        ...
        if not self._freq_is_pe and not self._q_is_pe:
            ...
            b0, b1, b2, a1, a2 = self._compute_coefficients(
                np.array([self._frequency]), np.array([self._q])
            )
            y = self._filter_constant_coeffs(x, b0[0], b1[0], b2[0], a1[0], a2[0])
        else:
    ...
    def _filter_constant_coeffs(
        ...
        y, self._lfilter_state = signal.lfilter(b, a, x, axis=0, zi=self._lfilter_state)
