...
class PortamentoPE(SourcePE):
    ...
    def __init__(
        ...
        if max_ramp_seconds < 0:
            raise ValueError(
                f"PortamentoPE: max_ramp_seconds must be non-negative (got {max_ramp_seconds})"
            )

        if not (0.0 <= ramp_fraction <= 1.0):
            raise ValueError(
                f"PortamentoPE: ramp_fraction must be between 0 and 1 (got {ramp_fraction})"
            )
    ...
    def _build_sequence(self) -> None:
        ...
        max_ramp_samples_resolved = max(
            1, int(round(self._max_ramp_seconds * self.sample_rate))
        )
        ...
        for i in range(len(self._notes) - 1):
            ...
            adaptive_ramp_samples = min(
                max_ramp_samples_resolved,
                int(round(curr_duration * self._ramp_fraction)),
            )
            ...
            if len(self._notes) == 2:
                ...
                cropped_ramp = CropPE(
                    ramp, 0, crop_duration, extend_mode=ExtendMode.HOLD_FIRST
                )
                ...
                crop_duration = next_ramp_start - curr_start
                cropped_ramp = CropPE(
                    ramp, 0, (crop_duration) - (0), extend_mode=ExtendMode.ZERO
                )
