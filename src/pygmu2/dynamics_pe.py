...
class DynamicsPE(ProcessingElement):
    ...
    def _compute_compression_gain(
        ...
        if knee <= 0:
            ...
            gain_db = np.where(
                below_knee, 0.0, np.where(above_knee, full_gain, knee_gain)
            )
    ...
    def _compute_expansion_gain(
        ...
        if knee <= 0:
            ...
            gain_db = np.where(
                above_knee, 0.0, np.where(below_knee, full_gain, knee_gain)
            )
    ...
    def _compute_gate_gain(
        ...
        if knee <= 0:
            ...
            gain_db = np.where(
                above_knee, 0.0, np.where(below_knee, range_db, knee_gain)
            )
    ...
    def __repr__(self) -> str:
        makeup_str = (
            "auto" if self._makeup_gain == self.AUTO else f"{self._makeup_gain_db:.1f}"
        )
        return (
