...
class LoopPE(ProcessingElement):
    ...
    def _resolve_loop_boundaries(self) -> None:
        ...
        if self._resolved_end is None:
            if self._loop_end is not None:
                ...
                raise ValueError(
                    "Cannot loop source with infinite extent without explicit loop_end"
                )
        ...
        if self._resolved_start is not None and self._resolved_end is not None:
            ...
            if self._loop_length <= 0:
                raise ValueError(
                    f"Loop length must be positive, got {self._loop_length}"
                )
    ...
    def __repr__(self) -> str:
        ...
        xfade_str = (
            f", crossfade_seconds={self._crossfade_seconds}"
            if self._crossfade_seconds
            else ""
        )
        return (
