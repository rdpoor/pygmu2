...
class SinePE(ProcessingElement):
    ...
    def _has_pe_inputs(self) -> bool:
        ...
        return (
            isinstance(self._frequency, ProcessingElement)
            or isinstance(self._amplitude, ProcessingElement)
            or isinstance(self._phase, ProcessingElement)
        )
    ...
    def _render(self, start: int, duration: int) -> Snippet:
        ...
        amp_values = self._scalar_or_pe_values(
            self._amplitude, start, duration, dtype=np.float64
        ).reshape(-1, 1)
        phase_mod = self._scalar_or_pe_values(
            self._phase, start, duration, dtype=np.float64
        )

        # Calculate phase
        if self._has_pe_inputs():
            # Non-pure: integrate frequency, use state
            phase = self._compute_phase_stateful(
                freq_values, phase_mod, start, duration
            )
        else:
