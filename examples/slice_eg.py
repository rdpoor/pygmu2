...
def extract_slices() -> dict:
    ...
    slices = {
        ...
        "cowbell": GainPE(
            SlicePE(drums_stream, start=ss(1.813), duration=ss(0.131)), 2.0
        ),
...
def demo_slice_polka():
    ...
    mix_stream = LoopPE(
        measure_stream, loop_start=ss(0), loop_end=ss(4.0 * SECONDS_PER_BEAT), count=4
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print()
        choice = (
            input(f"Choose a demo (1-{len(demos)-1} or 'a' for all): ").strip().lower()
        )
        print()
