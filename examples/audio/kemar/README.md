...
2. **Unzip** directly into this directory (`examples/audio/kemar`). The archive already contains `.wav` files—no conversion is required.

   If your archive has only `.dat` files, use the conversion script instead:
   ```bash
   python scripts/convert_kemar_to_wav.py --input /path/to/extracted_dat --output examples/audio/kemar
   ```
