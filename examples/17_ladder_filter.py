...
print("=== pygmu2 Example 17: Ladder Filter ===", flush=True)
...
def cutoff_sweep():
    ...
    sweep_stream = LadderPE(
        source_stream,
        frequency=cutoff_sweep_stream,
        resonance=0.3,
        mode=LadderMode.LP12,
        drive=1.2,
    )
    sweep_stream = GainPE(sweep_stream, gain=0.8)
    sweep_out_stream = CropPE(sweep_stream, 0, (duration_samples) - (0))

    pg.play(pg.GainPE(sweep_out_stream, gain=3.07))


DEMOS = [
    ("Original signal", original_signal),
    ("Ladder lowpass (800 Hz, resonance 0.6)", resonant_lowpass),
    ("Ladder sweep (200 Hz -> 4 kHz)", cutoff_sweep),
]
