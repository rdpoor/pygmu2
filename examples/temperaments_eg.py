...
def note_to_midi(note_name):
    ...
    if len(note_name) > idx and note_name[idx] in "#♯":
        ...
        len(note_name) > idx and note_name[idx] in "bB♭"
    ):  # Accept both 'b' and 'B' for flat
        semitone -= 1
        idx += 1
...
def play_chord(
    notes, temperament_name, temperament, start_time, duration=CHORD_DURATION
):
    """
    ...
    for note in notes:
        ...
        sine = SinePE(
            frequency=freq, amplitude=0.2 / len(notes)
        )  # Scale by number of notes
        sines.append(sine)
    ...
    return CropPE(
        chord, start_sample, (start_sample + duration_samples) - (start_sample)
    )
...
def demo_fifth_stack():
    ...
    play_chord_comparison(
        "5th Stack", notes_to_midi(["C3", "G3", "D4", "A4"]), gain=2.53
    )
