...
SLICE_FADE_OUT_SECONDS = 8.0
TRALFAM_TAIL_SECONDS = 5.0
TRALFAM_LOOP_COUNT = 6
...
def apply_post_blur_fade(pe):
    """Apply an equal-power gain envelope after the blur stage."""
...
def collect_processed_slices(sources, rng):
    ...
    for source_info in sources:
        ...
        for slice_num in range(SLICES_PER_FILE):
            ...
            paired = make_spatialized_slice_pair(ramped)
            print(
                ...
                f"pair_dur={paired.extent().end / file_sample_rate:.2f}s",
                flush=True,
            )
    return processed
...
def sequence_with_overlap(processed_slices, rng):
    ...
    for index, item in enumerate(play_order):
        ...
        if USE_OVERLAP and (fade_in_samples > 0 or fade_out_start is not None):
            pe_to_place = apply_sequence_crossfade(
                pe_to_place,
                fade_in_samples=fade_in_samples,
                fade_out_start=fade_out_start,
            )

        paired = make_spatialized_slice_pair(pe_to_place)

        positioned.append(pg.DelayPE(paired, delay=cursor))
        cursor += cursor_step
