...
def setup_fallback_configs():
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "SinePE (440 Hz)", lambda: SinePE(frequency=440.0), "source"
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "BlitSawPE (440 Hz, auto M)",
                lambda: BlitSawPE(frequency=440.0),
                "oscillator",
            ),
            BenchmarkConfig(
                "BlitSawPE (440 Hz, M=20)",
                lambda: BlitSawPE(frequency=440.0, m=20),
                "oscillator",
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "SuperSawPE (7 voices)",
                lambda: SuperSawPE(frequency=440.0, voices=7),
                "oscillator",
            ),
            BenchmarkConfig(
                "SuperSawPE (3 voices)",
                lambda: SuperSawPE(frequency=440.0, voices=3),
                "oscillator",
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "GainPE (constant)",
                lambda: GainPE(SinePE(frequency=440.0), gain=0.5),
                "transform",
            ),
            BenchmarkConfig(
                "GainPE (modulated)",
                lambda: GainPE(SinePE(frequency=440.0), gain=SinePE(frequency=5.0)),
                "transform",
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "DelayPE (1000 samples)",
                lambda: DelayPE(SinePE(frequency=440.0), delay=1000),
                "transform",
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "CropPE", lambda: CropPE(SinePE(frequency=440.0), 0, 44100), "transform"
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "MixPE (2 sources)",
                lambda: MixPE(SinePE(frequency=440.0), SinePE(frequency=550.0)),
                "transform",
            ),
            BenchmarkConfig(
                "MixPE (4 sources)",
                lambda: MixPE(
                    ...
                    SinePE(frequency=550.0),
                    SinePE(frequency=660.0),
                    SinePE(frequency=880.0),
                ),
                "transform",
            ),
        ],
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "LoopPE",
                lambda: LoopPE(CropPE(SinePE(frequency=440.0), 0, 4410)),
                "transform",
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "CompressorPE",
                lambda: CompressorPE(SinePE(frequency=440.0)),
                "dynamics",
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "LimiterPE", lambda: LimiterPE(SinePE(frequency=440.0)), "dynamics"
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "ExpanderPE", lambda: ExpanderPE(SinePE(frequency=440.0)), "dynamics"
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "EnvelopePE (RMS)",
                lambda: EnvelopePE(SinePE(frequency=440.0)),
                "analysis",
            ),
        ],
    )
    ...
    register_fallback(
        ...
        [
            BenchmarkConfig(
                "WindowPE (max)", lambda: WindowPE(SinePE(frequency=440.0)), "analysis"
            ),
        ],
    )
    ...
    if has_biquad:
        register_fallback(
            ...
            [
                ...
                BenchmarkConfig(
                    "BiquadPE (lowpass, modulated freq)",
                    lambda: BiquadPE(
                        SinePE(frequency=440.0),
                        mode=BiquadMode.LOWPASS,
                ...
                BenchmarkConfig(
                    "BiquadPE (bandpass, modulated Q)",
                    lambda: BiquadPE(
                        SinePE(frequency=440.0),
                        mode=BiquadMode.BANDPASS,
                        frequency=1000.0,
                        q=SinePE(frequency=2.0, amplitude=1.0),
                    ),
                    "filter",
                ),
            ],
    ...
    if has_svfilter:
        register_fallback(
            ...
            [
                ...
                BenchmarkConfig(
                    "SVFilterPE (lowpass, modulated freq)",
                    lambda: SVFilterPE(
                        SinePE(frequency=440.0),
                        mode=BiquadMode.LOWPASS,
                        frequency=SinePE(frequency=5.0, amplitude=500.0),
...
def run_benchmarks(
    ...
    for category in sorted(by_category.keys()):
        ...
        for config in by_category[category]:
            try:
                result = benchmark_pe(
                    config, num_runs=num_runs, warmup_runs=warmup_runs
                )
                results.append(result)
                print(
                    f"  {result.name}: {result.mean_time_ms:.3f} ms ({result.realtime_ratio:.1f}x realtime)"
                )
            except Exception as e:
...
def print_summary(results: list[BenchmarkResult]) -> None:
    ...
    for r in sorted_results:
        print(
            f"{r.name:<35} {r.mean_time_ms:>10.3f} {r.std_time_ms:>10.3f} {r.realtime_ratio:>10.1f}x"
        )
    ...
    for r in sorted_results[:5]:
        if r.realtime_ratio < 100:
            ...
            status = "FAST"
        print(
            f"  {r.name}: {r.mean_time_ms:.3f} ms/render, {r.realtime_ratio:.1f}x realtime [{status}]"
        )
...
def run_scaling_test() -> None:
    ...
    print(
        f"{'Buffer Size':>12} {'BlitSaw (ms)':>14} {'SuperSaw (ms)':>14} {'Ratio':>10}"
    )
    print("-" * 55)

    for buf_size in buffer_sizes:
        blit_config = BenchmarkConfig(
            "BlitSaw", lambda: BlitSawPE(frequency=440.0), "test"
        )
        super_config = BenchmarkConfig(
            "SuperSaw", lambda: SuperSawPE(frequency=440.0, voices=7), "test"
        )

        blit_result = benchmark_pe(
            blit_config, samples_per_run=buf_size, num_runs=100, warmup_runs=10
        )
        super_result = benchmark_pe(
            super_config, samples_per_run=buf_size, num_runs=100, warmup_runs=10
        )

        ratio = (
            super_result.mean_time_ms / blit_result.mean_time_ms
            if blit_result.mean_time_ms > 0
            else 0
        )

        print(
            f"{buf_size:>12} {blit_result.mean_time_ms:>14.4f} {super_result.mean_time_ms:>14.4f} {ratio:>10.2f}x"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scaling", action="store_true", help="Run buffer size scaling test"
    )
    parser.add_argument("--quick", action="store_true", help="Quick mode (fewer runs)")
    parser.add_argument(
        "--list",
        action="store_true",
        help="List discovered PEs without running benchmarks",
    )
